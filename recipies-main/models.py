...
class Recipie(db.Model):
    ...
    @classmethod
    def add_new_recipie(cls, recipie_data):
        recipie_to_add = cls(name=recipie_data["name"], description=recipie_data["description"], time=recipie_data["time"], user_id = session["user_id"])
        db.session.add(recipie_to_add)
        db.session.commit()
        return recipie_to_add